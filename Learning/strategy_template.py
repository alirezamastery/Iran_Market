...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.final_symbols = [0] * self.params.portfo
        self.position_flag = []
        # print(self.final_symbols)
        self.sym_count = 0
        self.RS_min = 100
    ...
    def next_open(self):
        ...
        if self.params.portfo > self.sym_count:
            sizes = sys_math.floor(0.95 * self.broker.get_value())

        x = 20

        size = sys_math.floor(sizes / self.datas[x].close[0])
        # open positions:

        if not self.buyflag:
            # print(size)
            print(self.broker.get_value() - (size * self.datas[x].close[0]))
            self.order = self.buy(data=self.datas[x])
            self.log('BUY CREATE: %s , Price %.2f' % (self.symbol_key[self.datas[x]] , self.datas[x].close[0]))
...
if __name__ == '__main__':
    ...
    # add the sizer
    # if args.commsizer:
    cerebro.addsizer(maxRiskSizerComms , debug=True , risk=1)
    # else:
