...
class analyze(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.InPosition = ['REMAPNA' , 'SHETRAN' , 'VATEJARAT']
    ...
    def next(self):
        if len(self.data) == self.data.buflen():
            for i , d in enumerate(self.datas):
                ...
                sell_condition = self.inds[d]['RSI'].rsi[0] < 70 and \
                                 self.inds[d]['RSI'].rsi[0] < self.inds[d]['RSI'].rsi[-1]
                if self.datas[i]._name in self.InPosition and sell_condition:
                    self.sell_signal.append([self.datas[i]._name ,
                                             round(self.inds[d]['RSI'].rsi[0] , 2) ,
                                             round(self.inds[d]['RS'].RS[0] , 2) ,
                                             round(self.inds[d]['RS'].MA[0] , 2)])
                # check for buy signals:
                buy_condition = 70 < self.inds[d]['RSI'].rsi[0] < 80 and \
                                self.inds[d]['RSI'].rsi[0] > self.inds[d]['RSI'].rsi[-1] and \
                                self.inds[d]['RS'].RS[0] > self.inds[d]['RS'].MA[0]
                if buy_condition:
    ...
    def stop(self):
        ...
        with open('analyze_log.txt', 'w' , encoding='utf-8') as log_file:
            ...
            for i , d in enumerate(self.sell_signal):
                ...
                out_text = f'{d[0]:15.15} │ RSI: {d[1]:<5} │ RS: {d[2]:<6} │ MA: {d[3]:<6}'
...
class analyzing:
    ...
    input_list = '../Lists/all_files.txt'
