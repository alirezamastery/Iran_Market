...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.open_syms = [[-1, -1] for x in range(self.portfo)]
        self.portfo_syms = []
    ...
    def next_open(self):
        ...
        if self.inserted[1]:
            ...
            print('cash:', self.broker.get_cash(), 'sizes:', sizes)
        ...
        mean_var = np.mean(self.variance)
        var_var = np.var(self.variance)
        ...
        for i, d in enumerate(self.datas):
            if d._name == 'TEPIX':
                pass
            elif (self.inds[d]['RS'].MA[0]) * self.s_th < self.inds[d]['RS'].RS[0] and \
        ...
        opens = []
        vars = []
        # print('sym_count_e',self.sym_count_e)
        if self.sym_count_e >= self.portfo:
            for i, d in enumerate(self.position_flag):
                ...
                # number.append(d[0])
            # print('vars:', len(vars))
            # print(self.position_flag)
            if len(self.irs) > 0 and max(vars) < self.variance[self.irs[0][0]]:
...
if __name__ == '__main__':
    ...
    # load data:
    sd.add_cerebro_data(cerebro , start='2010-01-01' , end='2020-01-01' , show=[])
